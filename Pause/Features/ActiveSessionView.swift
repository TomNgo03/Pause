            ...
            TimelineView(.periodic(from: .now, by: 1)) { context in
                ...
                VStack(spacing: 26) {
                    ...
                    ZStack {
                        ...
                        VStack(spacing: 6) { Text(remaining(at: context.date)).font(.system(size: 48, weight: .bold, design: .rounded)).monospacedDigit(); Text("until arrival").font(.subheadline).foregroundStyle(.white.opacity(0.66)) }
    ...
    var body: some View {
        Canvas { context, size in
            for index in 0..<70 {
                ...
                context.fill(Path(ellipseIn: CGRect(x: x, y: y, width: radius * 2, height: radius * 2)), with: .color(.white.opacity(index.isMultiple(of: 3) ? 0.72 : 0.35)))
            }
...
private struct Planet: View {
    let progress: Double
    var body: some View {
        Circle().fill(RadialGradient(colors: [PauseTheme.violet.opacity(0.88), PauseTheme.indigo.opacity(0.70), PauseTheme.ink], center: .topLeading, startRadius: 10, endRadius: 150))
            .overlay(alignment: .topLeading) { Circle().fill(.white.opacity(0.10)).frame(width: 46).offset(x: 48, y: 55) }
            .overlay(alignment: .bottomTrailing) { Circle().fill(.black.opacity(0.16)).frame(width: 33).offset(x: -52, y: -48) }
